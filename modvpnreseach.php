...
if (!empty($vpn) && !empty($site))
{
        $vpnname=$vpn;
	$site=$site;
        echo '<form action="modvpnreseach.php" method="post">';
	...
	while($res=mysql_fetch_assoc($req))
  	{
		$vpnname=$res['vpnname'];
		echo '
		<form action="modvpnreseach.php" method="post">';
echo '<!-- Table markup-->';
                echo '<table id="rounded-corner">';
echo '<!-- Table header-->';
echo '<thead>';
                echo '<tr><th scope="col" class="rounded-company"><div align="left">Modify VPN <b>'.$vpnname.'</b> on site <b>'.$site.'</b> </br>If you change secrets key, right public IP or right ID modification will be apply to all VPN on this site</div></th><th scope="col" class="rounded-q4"></th></tr>';
echo '</thead>';
echo '<!-- Table footer-->';
echo '<tfoot>';
...
//echo "$confkey - $siteconf - $left - $leftid - $leftsubnet - $right - $rightid - $secretkey - $rightsubnet - $usedtemplate - $vpnname";
        	$command=("./scriptvpn.sh vpnupdate $siteconf $vpnname $left $leftid $leftsubnet $right $rightid $rightsubnet $secretkey $usrlvl");
//	        echo exec($command);
